...
| 路径 | 内容 |
|---|---|
| `alignment/fixtures/<name>.json` | 模型行为 fixture(scripted 手写 / replay 录制) |
| `alignment/seeds/<path>` | case 需要的种子文件(会拷进沙箱 cwd) |
| `alignment/cases/<group>/<name>/case.json` | case 定义 |
| `alignment/golden/<group>/<name>/golden.json` | 从 TS 录出的期望输出 |
...
1. 写 `alignment/cases/<g>/<n>/case.json`,加 `"kind": "deterministic"`。
2. 跑一次 hare 看输出:`python scripts/e2e_runner.py alignment/cases/<g>/<n>/case.json`。
3. 人工核对后把输出冻进 `alignment/golden/<g>/<n>/golden.json`(至少 `case_id`/`status`/`state.exit_code`/`stdout`)。
4. `pytest tests/e2e -k <case_id>` 跑绿。

### B. 模型驱动 case(单轮问答 / 工具调用 / 权限 / 压缩)

1. 写 `alignment/fixtures/<name>.json`。
2. 写 `case.json`,声明 `"fixture": "alignment/fixtures/<name>.json"`;需要文件的加 `"fs": {"seed": [...]}`。
...
python scripts/mock_anthropic_server.py alignment/fixtures/<name>.json 8089  # 手动起 mock
```
