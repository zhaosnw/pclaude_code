...
VERIFY = PROJECT_ROOT / "scripts" / "verify_alignment.py"
GEN_PRIORITY = PROJECT_ROOT / "scripts" / "gen_alignment_priority.py"


def test_alignment_data_uses_canonical_python_paths() -> None:
    data = json.loads(
        (PROJECT_ROOT / "legacy_alignment" / "alignment_data.json").read_text(
            encoding="utf-8"
        )
    )
    bad = []
...
def test_gen_alignment_priority_is_idempotent() -> None:
    before = (
        PROJECT_ROOT / "legacy_alignment" / "alignment_data.json"
