...
import { isHareBridgeMode } from '../utils/envUtils.js'
...
export function startBackgroundCacheRefresh(): void {
  if (isHareBridgeMode()) {
    // Delay file indexing so it doesn't contend with bridge init on first render.
    setTimeout(() => {
      if (!fileListRefreshPromise) startBackgroundCacheRefresh_impl();
    }, 5000);
    return;
  }
  startBackgroundCacheRefresh_impl();
}
function startBackgroundCacheRefresh_impl(): void {
