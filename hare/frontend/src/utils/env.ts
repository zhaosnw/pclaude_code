...
export const getGlobalClaudeFile = memoize((): string => {
  ...
  const hareFilename = `.hare${fileSuffixForOauthConfig()}.json`
  const legacyClaudeFilename = `.claude${fileSuffixForOauthConfig()}.json`
  const legacyGlobal = join(process.env.CLAUDE_CONFIG_DIR || homedir(), legacyClaudeFilename)

  if (getFsImplementation().existsSync(legacyGlobal)) {
    return legacyGlobal
  }

  return join(configHome, hareFilename)
})
